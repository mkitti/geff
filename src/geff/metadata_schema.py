            ...
            if self.type == "space":
                if validate_space_unit(self.unit):
                    (self.unit in VALID_SPACE_UNITS) or warnings.warn(
                        f"""Spatial unit {self.unit} not in valid OME-Zarr
                        units {VALID_SPACE_UNITS}. Reader applications may not
                ...
                if validate_time_unit(self.unit):
                    (self.unit in VALID_TIME_UNITS) or warnings.warn(
                        f"""Temporal unit {self.unit} not in valid OME-Zarr
                        units {VALID_TIME_UNITS}. Reader applications may not
                        know what to do with this information.""",
                        stacklevel=2,
                    )
                else:
